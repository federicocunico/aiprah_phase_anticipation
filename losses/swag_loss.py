...
class SWAGLoss(nn.Module):
    ...
    def forward(
        self, current_logits, future_logits, regression_logits, current_targets, future_targets, regression_targets
    ):
        """
        ...
        Args:
            ...
            current_targets (torch.Tensor): [B, C], one-hot ground truth for current classification.
            future_targets (torch.Tensor): [B, T, F, C], binary ground truth for future anticipation.
            regression_targets (torch.Tensor): [B, T, C], ground truth for remaining time regression.

        Returns:
            dict: Dictionary with individual loss components and total loss.
        """
        # Current phase classification loss
        classification_loss = self.ce_loss(current_logits, current_targets.argmax(dim=-1))

        # Future phase anticipation loss
        future_loss = self.bce_loss(future_logits, future_targets.float())

        # Regression loss for remaining time
        regression_loss = self.mae_loss(regression_logits, regression_targets)

        # R2C mapping and classification
        r2c_probs = self.r2c_mapping(regression_logits)  # [B, T, F, C]
        r2c_loss = self.bce_loss(r2c_probs, future_targets.float())
