...
class TemporalCNNAnticipation(nn.Module):
    ...
    def forward(
        ...
        # FIXED: Conditional clamping - only during inference
        if self.training:
            # During training: no clamping - let model learn naturally
            anticipation = anticipation_raw
        else:
            # During inference: apply constraints for safety/plotting
...
PRINT_EVERY = 20
CKPT_PATH = Path("peg_and_ring_cnn_noclamp.pth")
...
# UPDATED: Improved loss function with soft penalties
def compute_training_loss(reg_pred, reg_target, logits, labels, time_horizon):
    """
    Enhanced loss with soft constraints instead of hard clamping
    """
    # Main losses
    anticipation_loss = F.smooth_l1_loss(reg_pred, reg_target)
    phase_loss = F.cross_entropy(logits, labels)

    # Soft penalties to guide learning without hard constraints
    # Small penalty for predictions that go too far beyond horizon
    overflow_penalty = F.relu(reg_pred - time_horizon * 1.5).mean() * 0.05

    # Small penalty for very negative predictions (though Softplus should prevent this)
    negative_penalty = F.relu(-reg_pred).mean() * 0.02

    # Row-min penalty: encourage at least one phase to be near 0 (current phase)
    row_min_values = reg_pred.min(dim=1)[0]  # [B]
    row_min_penalty = F.relu(row_min_values - 0.2).mean() * 0.03
    ...
    return total_loss, anticipation_loss, phase_loss
...
def evaluate(
    ...
    for _, (frames, meta) in enumerate(loader):
        ...
        train_loss, anticipation_loss, ce = compute_training_loss(
            reg, ttnp, logits, labels, TIME_HORIZON
        )
    ...
    if True:
        for epoch in range(1, EPOCHS + 1):
            ...
            for it, (frames, meta) in enumerate(train_loader, start=1):
                ...
                train_loss, anticipation_loss, phase_loss = compute_training_loss(
                    reg, ttnp, logits, labels, TIME_HORIZON
                )
