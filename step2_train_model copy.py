...
def test(sequence_length, num_classes, chkpt_dst, test_loader, device, backbone=None):
    ...
    mb: MemoryBank = create_memorybank(
        ...
        device=device,
        membank_size=len(train_loader.dataset),
    )

    model = TemporalResNetLSTM(
        backbone=backbone, memory_bank=mb, sequence_length=sequence_length, num_classes=num_classes
    )
    model.to(device)
    ...
    plt.figure()
    ax = plt.subplot(111)
    ax.set_ylabel("Time to next phase (cap 5)")
    ax.set_xlabel("Frames")
    ax.plot(np.asarray(y_true_regr), label="True", color="blue")
    ax.plot(np.asarray(y_pred_regr), label="Predicted", color="red")
    plt.legend()
    plt.savefig(chkpt_dst.replace(".pth", ".png"))
...
def train_model():
    ...
    fig = plt.figure()
    ax = plt.subplot(111)  # Create a subplot for timings
    ...
    for e in range(epochs):
        ...
        for i, batch in tqdm(enumerate(train_loader), total=l, desc=f"Epoch {e}"):
            ...
            current_phase, anticipated_phase = model(frames)
            # scale anticipated phase, cut if it is greater than model.time_horizon (default=5 mintues)
            time_to_next_phase = torch.clamp(time_to_next_phase, 0, model.time_horizon)
