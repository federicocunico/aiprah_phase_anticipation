...
MODEL_PATH = "/home/saras/saras/linux/src/aiprah/phase_anticpation/src/peg_and_ring_slowfast_transformer_BEST.pth"  # checkpoint to load
...
class AnticipationNode:
    def __init__(self):
        ...
        self.sub = rospy.Subscriber(
            IMAGE_TOPIC, RosImage, self.image_cb, queue_size=QUEUE_SIZE_SUB
        )
    ...
    def image_cb(self, msg: RosImage):
        ...
        if self.last_proc_time is not None and (
            now - self.last_proc_time < self.min_period
        ):
            return
        ...
        with ctx:
            frames = (
                torch.stack(list(self.frame_buf), dim=0).unsqueeze(0).to(self.device)
            )
            # If your model signature changed, adapt here accordingly.
            # We assume it returns: reg, logits_ignored, completion, other_ignored
            reg, _logits_unused, completion_perc, _ = self.model(frames)  # noqa: F841
            reg = reg.squeeze(0)  # [C]
        ...
        # publish
        self.pub_anticip.publish(Float32MultiArray(data=reg_raw_list))
        self.pub_phase.publish(Int32(data=int(phase_proc_int)))
        self.pub_phase_prob.publish(Float32(data=confidence_scalar))
        self.pub_completion.publish(Float32(data=float(completion_scalar)))
