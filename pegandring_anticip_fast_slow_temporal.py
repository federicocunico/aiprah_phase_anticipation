...
# -----------------------------
# Slow-Fast Anticipation Model (with history fusion)
# -----------------------------
class SlowFastTemporalAnticipation(nn.Module):
    """
    Slow–Fast temporal anticipation model with completion head and
    **anticipation history fusion**.
    ...
    def __init__(
        ...
        *,
        ...
        # ---- Anticipation history encoder & fusion (NEW) ----
        self.ant_history_rnn = nn.GRU(
            input_size=num_classes,
            hidden_size=hidden_channels,
            num_layers=1,
            batch_first=True,
        )
        self.ant_history_proj = nn.Sequential(
            nn.LayerNorm(hidden_channels),
            nn.Linear(hidden_channels, hidden_channels),
            nn.GELU(),
            nn.Dropout(dropout),
        )
        # Gated residual fusion for current token and pooled token
        self.hist_fusion_gate_current = nn.Linear(2 * hidden_channels, hidden_channels)
        self.hist_fusion_gate_pooled = nn.Linear(2 * hidden_channels, hidden_channels)
            ...
            for m in module.modules():
                if isinstance(m, nn.Linear):
                    ...
                    if m.bias is not None:
                        nn.init.zeros_(m.bias)
        # Gates
        nn.init.xavier_uniform_(self.hist_fusion_gate_current.weight)
        nn.init.zeros_(self.hist_fusion_gate_current.bias)
        nn.init.xavier_uniform_(self.hist_fusion_gate_pooled.weight)
        nn.init.zeros_(self.hist_fusion_gate_pooled.bias)
    ...
    def _encode_history(
        ...
        """
        Returns a [B, H] embedding from prev_anticipation history.
        If absent or empty, returns zeros.
        """
        if (meta is None) or ("prev_anticipation" not in meta):
            return torch.zeros(batch_size, self.hidden_channels, device=device)

        prev_ant = meta["prev_anticipation"]
        if not isinstance(prev_ant, torch.Tensor):
            prev_ant = torch.tensor(prev_ant, dtype=torch.float32)
        if prev_ant.ndim == 2:
            # Unbatched -> add batch dim
            prev_ant = prev_ant.unsqueeze(0)
        prev_ant = prev_ant.to(device)

        if prev_ant.numel() == 0 or prev_ant.shape[1] == 0:
            return torch.zeros(batch_size, self.hidden_channels, device=device)

        # GRU encoding → take final hidden
        _, h_n = self.ant_history_rnn(prev_ant)  # [1,B,H]
        hist_emb = h_n.squeeze(0)                # [B,H]
        hist_emb = self.ant_history_proj(hist_emb)
        return hist_emb
    ...
    def forward(
        ...
        # Conformer temporal encoder (on fast timeline)
        encoded = self.temporal_encoder(fused_fast.transpose(1, 2))  # [B,T,H]

        # --- NEW: encode anticipation history and fuse ---
        hist_emb = self._encode_history(meta, device=device, batch_size=B)  # [B,H]

        # Heads use:
        current = encoded[:, -1, :]  # [B,H]
        # gated residual fusion for current token
        g_cur = torch.sigmoid(
            self.hist_fusion_gate_current(torch.cat([current, hist_emb], dim=1))
        )
        current = current + g_cur * hist_emb  # [B,H]

        # Classification & completion heads (use fused "current")
        phase_logits = self.phase_head(current)
        completion = self.completion_head(current)

        # Anticipation via attention pooling
        query = self.anticipation_query.expand(B, -1, -1)  # [B,1,H]
        pooled, _ = self.temporal_attention_pool(query=query, key=encoded, value=encoded)
        pooled = pooled.squeeze(1)  # [B,H]

        # also fuse history into pooled token
        g_pool = torch.sigmoid(
            self.hist_fusion_gate_pooled(torch.cat([pooled, hist_emb], dim=1))
        )
        pooled = pooled + g_pool * hist_emb  # [B,H]

        raw = self.anticipation_head(pooled)  # [B,C]
...
def evaluate(
    ...
    for _, (frames, meta) in enumerate(loader):
        ...
        # NOTE: meta tensors that are used inside the model are moved on-demand
        # in model.forward(). No extra device moves are needed here.
...
def _try_resume(
    ...
    if RESUME_IF_CRASH and LAST_CKPT_PATH.exists():
        try:
            ...
            # RNG restore intentionally disabled for reproducibility differences across envs
