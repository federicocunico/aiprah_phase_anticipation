...
INCLUDE_NULL_IN_AP = True  # Include NULL verb/target in AP calculations (part of the task)
...
class TrainingPlotter:
    ...
    def _plot_triplet_map(self):
        ...
        for key, label, color in metrics_to_plot:
            if key in self.history:
                plt.plot(self.epochs, self.history[key], label=label, color=color, linewidth=2, marker='o', markersize=4)
        
        plt.xlabel('Epoch', fontsize=12)
        plt.ylabel('Average Precision (AP)', fontsize=12)
        plt.title(f'Stage {self.stage} - Comprehensive Triplet AP Metrics', fontsize=14, fontweight='bold')
        plt.legend(loc='best', fontsize=10)
        plt.grid(True, alpha=0.3)
        plt.ylim(0, 1.0)  # AP is between 0 and 1
        plt.tight_layout()
        plt.savefig(self.save_dir / f"stage{self.stage}_triplet_ap_comprehensive.png", dpi=150, bbox_inches='tight')
        plt.close()
        ...
        plt.xlabel('Epoch')
        plt.ylabel('mAP')
        plt.title(f'Stage {self.stage} - Triplet mAP Metrics (Legacy)', fontsize=14, fontweight='bold')
        plt.legend(loc='best')
        plt.grid(True, alpha=0.3)
        plt.ylim(0, 1.0)
        plt.tight_layout()
        ...
        if len(all_left_verb_preds) > 0:
            ...
            for v_idx in range(num_verb_classes):
                for t_idx in range(num_target_classes):
                    ...
                    binary_true = ((all_verb_gts == v_idx) & (all_target_gts == t_idx)).astype(int)
                    # Only compute AP if this combination exists in ground truth
                    if np.sum(binary_true) > 0:
                        try:
                            # Combine probabilities: P(verb=v_idx) * P(target=t_idx)
                            combined_probs = all_verb_probs[:, v_idx] * all_target_probs[:, t_idx]
            ...
            print(f"    AP Calculation ({null_status}): {len(verb_aps)} verb classes, {len(target_aps)} target classes, {len(it_aps)} verb-target pairs")
...
class BestMetricsTracker:
    ...
    def update(self, stage: int, val_stats: Dict[str, Any]) -> Dict[str, bool]:
        ...
        if stage == 1:
            ...
            if 'triplet_ap_i' in val_stats:
                if val_stats['triplet_ap_i'] > self.best_metrics[stage_key]['best_ap_i']:
                    self.best_metrics[stage_key]['best_ap_i'] = float(val_stats['triplet_ap_i'])
                    improved['ap_i'] = True
            
            if 'triplet_ap_v' in val_stats:
                if val_stats['triplet_ap_v'] > self.best_metrics[stage_key]['best_ap_v']:
                    self.best_metrics[stage_key]['best_ap_v'] = float(val_stats['triplet_ap_v'])
                    improved['ap_v'] = True
            
            if 'triplet_ap_t' in val_stats:
                if val_stats['triplet_ap_t'] > self.best_metrics[stage_key]['best_ap_t']:
                    self.best_metrics[stage_key]['best_ap_t'] = float(val_stats['triplet_ap_t'])
                    improved['ap_t'] = True
            
            if 'triplet_ap_iv' in val_stats:
                if val_stats['triplet_ap_iv'] > self.best_metrics[stage_key]['best_ap_iv']:
                    self.best_metrics[stage_key]['best_ap_iv'] = float(val_stats['triplet_ap_iv'])
                    improved['ap_iv'] = True
            
            if 'triplet_ap_it' in val_stats:
                if val_stats['triplet_ap_it'] > self.best_metrics[stage_key]['best_ap_it']:
                    self.best_metrics[stage_key]['best_ap_it'] = float(val_stats['triplet_ap_it'])
                    improved['ap_it'] = True
            
            if 'triplet_ap_ivt' in val_stats:
                if val_stats['triplet_ap_ivt'] > self.best_metrics[stage_key]['best_ap_ivt']:
                    self.best_metrics[stage_key]['best_ap_ivt'] = float(val_stats['triplet_ap_ivt'])
    ...
    def save_latex_table(self):
        ...
        with open(latex_path, 'w') as f:
            ...
            f.write(f"% AP_i (Instrument/Verb):    {s2['best_ap_i']:.4f} ({s2['best_ap_i']*100:.2f}%)\n")
            f.write(f"% AP_v (Verb):                {s2['best_ap_v']:.4f} ({s2['best_ap_v']*100:.2f}%)\n")
            f.write(f"% AP_t (Target):              {s2['best_ap_t']:.4f} ({s2['best_ap_t']*100:.2f}%)\n")
            f.write(f"% AP_iv (Instrument+Verb):    {s2['best_ap_iv']:.4f} ({s2['best_ap_iv']*100:.2f}%)\n")
            f.write(f"% AP_it (Instrument+Target):  {s2['best_ap_it']:.4f} ({s2['best_ap_it']*100:.2f}%)\n")
            f.write(f"% AP_ivt (Complete Triplet):  {s2['best_ap_ivt']:.4f} ({s2['best_ap_ivt']*100:.2f}%)\n")
...
def train_stage(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if stage == 2 and compute_triplets and val_stats.get("triplet_samples", 0) > 0:
            ...
            if 'triplet_ap_i' in val_stats:
                print(val_summary)
                print(f"           AP Metrics: AP_i={val_stats['triplet_ap_i']:.4f} AP_v={val_stats['triplet_ap_v']:.4f} "
                      f"AP_t={val_stats['triplet_ap_t']:.4f}")
                print(f"                       AP_iv={val_stats['triplet_ap_iv']:.4f} AP_it={val_stats['triplet_ap_it']:.4f} "
        ...
        if plotter is not None:
            ...
            if compute_triplets and val_stats.get("triplet_samples", 0) > 0:
                ...
                if stage == 2:
                    if 'triplet_ap_i' in val_stats:
                        log_dict.update({
                            f"stage{stage}/val/triplet_ap_i": val_stats['triplet_ap_i'],
                            f"stage{stage}/val/triplet_ap_v": val_stats['triplet_ap_v'],
                            f"stage{stage}/val/triplet_ap_t": val_stats['triplet_ap_t'],
                            f"stage{stage}/val/triplet_ap_iv": val_stats['triplet_ap_iv'],
                            f"stage{stage}/val/triplet_ap_it": val_stats['triplet_ap_it'],
                            f"stage{stage}/val/triplet_ap_ivt": val_stats['triplet_ap_ivt'],
                        })
                    # Legacy mAP metrics
                    if 'triplet_verb_map' in val_stats:
                        log_dict.update({
            ...
            if best_metrics_tracker is not None:
                try:
                    ...
                    if stage == 1:
                        ...
                        print(f"  Best Val MAE:  {s1.get('best_val_mae', float('inf')):.4f}")
                        print(f"  Best Val Acc:  {s1.get('best_val_acc', 0.0):.4f}")
                        print(f"  Best inMAE:    {s1.get('best_inMAE', float('inf')):.4f}")
                        print(f"  Best wMAE:     {s1.get('best_wMAE', float('inf')):.4f}")
                    elif stage == 2:
